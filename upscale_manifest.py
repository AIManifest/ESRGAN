            ...
            for idx, img_path in enumerate(images, 1):
                ...
                for i, model_path in enumerate(model_chain):
                    ...
                    if self.cache_max_split_depth and len(split_depths.keys()) > 0:
                        ...
                        rlt, depth = ops.auto_split_upscale(
                            img, self.upscale, self.outscale
                        )
                        split_depths[i] = depth

                    final_scale *= self.outscale
...
def esrgan_upscale(esrganargs):
    model = esrganargs.model
    input = esrganargs.input
    output = esrganargs.output
    outscale = esrganargs.outscale
    reverse = esrganargs.reverse
    skip_existing = esrganargs.skip_existing
    delete_input = esrganargs.delete_input
    seamless = esrganargs.seamless
    cpu = esrganargs.cpu
    fp16 = esrganargs.fp16
    device_id = esrganargs.device_id
    cache_max_split_depth = esrganargs.cache_max_split_depth
    binary_alpha = esrganargs.binary_alpha
    ternary_alpha = esrganargs.ternary_alpha
    alpha_threshold = esrganargs.alpha_threshold
